...
#Extract the mutation rate data:
mut_rate_df <- cesa_samples_by_groups@mutrates

library(tidyverse)  
# change in mutation rate across stages
mut_rate_df <- mut_rate_df %>% 
  select(gene, rate_grp_2, rate_grp_3) %>% 
  mutate(p_1 = rate_grp_2 / rate_grp_3) %>% 
  mutate(p_2 = 1 - p_1)
  
# saving gene mutation rates into separate data frame:
Late_rate <- mut_rate_df %>%
  select(gene, rate = rate_grp_2) %>%
  data.table::setDT()
Meta_rate <- mut_rate_df %>%
  select(gene, rate = rate_grp_3) %>%
  data.table::setDT()
...
# change in mutation rate across stages
mut_rate_df <- mut_rate_df %>% 
  select(gene, rate_grp_1, rate_grp_3) %>% 
  mutate(p_1 = rate_grp_1 / rate_grp_3) %>% 
  mutate(p_2 = 1 - p_1)
  
  
# saving gene mutation rates into separate data frame:
Early_rate <- mut_rate_df %>%
  select(gene, rate = rate_grp_1) %>%
  data.table::setDT()
Meta_rate <- mut_rate_df %>%
  select(gene, rate = rate_grp_3) %>%
  data.table::setDT()
