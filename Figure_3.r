...
mut_rate_df <- tibble(gene = cesa_samples_by_groups$dNdScv_results$rate_grp_2$genemuts$gene_name,
                      exp_hyp_mu = cesa_samples_by_groups$dNdScv_results$rate_grp_2$genemuts$exp_syn_cv,
                      exp_cancer1_mu = cesa_samples_by_groups$dNdScv_results$rate_grp_3$genemuts$exp_syn_cv)

mut_rate_df$n_syn_sites = nsyn_sites[mut_rate_df$gene]

mut_rate_df %>% 
  mutate(hyp_mu = (exp_hyp_mu / n_syn_sites) / samples_in_Late_groups) %>%
  mutate(cancer1_mu = (exp_cancer1_mu / n_syn_sites) / samples_in_Metastasis_groups) %>%
...
mut_rate_df %>% 
  mutate(hyp_mu = (exp_hyp_mu / n_syn_sites) / samples_in_Early_groups) %>%
  mutate(cancer1_mu = (exp_cancer1_mu / n_syn_sites) / samples_in_Metastasis_groups) %>%
  mutate(cancer_greater = cancer1_mu > hyp_mu) -> 
  mut_rate_df
