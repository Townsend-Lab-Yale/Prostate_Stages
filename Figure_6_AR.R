...
# In two of these files, MAF2 and MAF3, the column "Gleason" is equal to "Empty" or "Early" or "Late" (there are no Metastasis), so they would be removed from the rest of analysis.
...
AR_final = ces_variant(AR4, variants = select_variants(AR4, min_freq = 2))
...
bargraph_AR_SI <- ggplot(data=PRAD_results_recurrent, aes(x=reorder(variant_name, -selection_intensity), y=selection_intensity, fill=selection_intensity))+
  ...
  xlab("AR amino-acid substitution (and observed count)") + ylab("Scaled selection coefficient") +
